...
export default function Order() {
          ...
          const fetchOrderSheet = async () => {
            ...
            try {
              ...
              const mappedItems = (data.orderItems || []).map((item) => {
                const qtyNum = parseQuantityNumber(item.quantity);      // "2개" → 
                const totalPriceNum = parsePriceNumber(item.totalPrice); // "20,000원" → 20000
               
                const itemUnitPrice = Math.floor(totalProductPrice / totalItemsCount);
                return {
                  id: item.cartItemId,
                  name: item.dishName,
                  price: itemUnitPrice,
  ...
  const handlePay = async () => {
    ...
    if (window.IMP) {
      ...
      window.IMP.request_pay(payParams, async function(rsp) {
          if (rsp.success) {
              ...
              try {
                  ...
                  alert(`결제는 되었으나 주문 생성에 실패했습니다. 고객센터에 문의해주세요. (PG사 주문번호: ${rsp.merchant_uid})`);
              }
